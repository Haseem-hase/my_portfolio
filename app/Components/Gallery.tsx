...
export type GalleryImage = string | ImageObject;
...
interface GalleryItem extends Coord {
  src: string;
  alt: string;
}

function buildItems(pool: GalleryImage[], seg: number): GalleryItem[] {
...
export default function Gallery({
  ...
  useEffect(() => {
    ...
    const step = () => {
      // Rotate only if:
      // 1. We are NOT dragging
      // 2. Inertia is NOT active (inertiaRAF.current is null)
      // 3. No image is focused (focusedElRef.current is null)
  ...
  const openItemFromElement = useCallback(
    (el: HTMLDivElement) => {
      ...
      if (!main || !frame || !viewer || tileR.width <= 0 || tileR.height <= 0) {
        openingRef.current = false;
        focusedElRef.current = null;
